...
getContracts = async function(){
	...
	while (true) {
		try {
			...
			var re_code_url = /href\=\'\/address\/(.+?)\#code\'/sg;
			var addresses = []
			...
			for(var address of addresses){
				...
				var re_code = /<pre\sclass\=\'js-sourcecopyarea.+?>(.+?)<\/pre>/sg;
				var m_code = re_code.exec(contract_list_body)
				

				var re_contract = /Name<\/span>:\s*<\/td>\s*<td>\s*(.+?)\s*<\/td>/sg;
				...
				if(m_contract !== null && m_code !== null){
					...
					var location = "./contracts/"+ address + "_" + contract_name  + ".sol"
